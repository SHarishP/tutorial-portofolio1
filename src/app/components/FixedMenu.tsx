...
const FixedMenu = () => {
  ...
  useEffect(() => {
    if (isMounted) {
      const handleScroll = () => {
        setShowMenuButton(window.scrollY > 50); //show the button after scrolling 150px
      };
      ...
      if (!isMobile) {
        ...
        // alaways show the button on mobile
        setShowMenuButton(true);
      }
    }
  }, [isMobile, isMounted]);

  //   Prevent renderiung until mounted
  if (!isMounted) return null;

  return (
    <div className="fixed w-full h-[400px] z-50 justify-center flex pointer-events-none bg-transparent">
      {/* menu */}
      <AnimatePresence>
        {showMenu && showMenuButton && (
          <motion.div
            ...
            className="relative w-full max-w-md lg:max-w-none h-[400px] bottom-[34rem] px-4 pointer-events-auto"
          >
            <div className="bg-white w-full h-full shadow-custom max-w-[1170px] mx-auto py-12 xl:py-12 px-12 xl:px-32 flex items-center gap-12 rounded-lg">
              <Nav
                containerStyles="lg:border-r border-secondary/20 lg:pr-12 w-full lg:w-auto text-center lg:text-left"
